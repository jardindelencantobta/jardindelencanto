...
> **Marca y contacto centralizados en `src/config/brand.ts`** (nombre, WhatsApp Business 312 866 1699, correo, Instagram, razón social). No hardcodear teléfonos, correos ni redes en componentes: importar `BRAND`, `WHATSAPP`, `waLink()`, `SOCIAL`. Redes activas: Instagram, TikTok y Facebook; las que tengan valor `null` no se renderizan.
