#!/usr/bin/env tsx
// Genera docs/manual-usuario.pdf desde docs/manual-usuario.md
// Uso: npx tsx scripts/generate-manual.tsx
...
const S = StyleSheet.create({
  ...
  // Ordered list
  olWrap: { marginBottom: 6 },
  olItem: { flexDirection: 'row', marginBottom: 3, paddingLeft: 4 },
  olNum:  { width: 18, fontFamily: 'Helvetica-Bold', fontSize: 9, color: G.medium, paddingTop: 0.5 },
...
function inline(raw: string): React.ReactNode[] {
  ...
  return parts.map((p, i) => {
    ...
    if (p.startsWith('`') && p.endsWith('`') && p.length > 2)
      return <Text key={i} style={{ fontFamily: 'Courier', fontSize: 7.5, backgroundColor: G.bgLight }}>{p.slice(1, -1)}</Text>
...
function parseMd(md: string): MdBlock[] {
  ...
  while (i < lines.length) {
    ...
    if (/^\d+\. /.test(l)) {
      ...
      out.push({ t: 'olist', items }); continue
    }
...
function RenderBlock({ b }: { b: MdBlock }) {
  switch (b.t) {
    ...
    // Ordered list — renders "1." "2." etc.
    case 'olist': return (
      <View style={S.olWrap}>
        {b.items.map((item, j) => (
          <View key={j} style={S.olItem}>
            <Text style={S.olNum}>{`${j + 1}.`}</Text>
            <Text style={S.olTxt}>{inline(item)}</Text>
          </View>
        ))}
      </View>
    )
...
// ─── PDF Document ─────────────────────────────────────────────────────────────
function ManualDoc({ blocks }: { blocks: MdBlock[] }) {
  return (
    <Document
      title="Manual de Usuario — Hacienda El Encanto"
      author="Ing. Jeisson Rincón"
      creator="Portal Hacienda El Encanto"
      subject="Manual de usuario del portal de gestión de eventos"
    >
      <CoverPage />
      ...
      <Page size="A4" style={S.page}>
        <View style={S.hdr} fixed>
          ...
          <Text style={S.hdrTitle}>Manual de Usuario — Hacienda El Encanto</Text>
...
// ─── Main ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log('📄  Leyendo manual-usuario.md...')
  ...
  const buf = await renderToBuffer(<ManualDoc blocks={blocks} />)
